...
void kf()
{
  ...
  roll_g = roll_g_1 + (Mpu6050_Data.Gyro_X + ((sin(pitch_g_1)*sin(roll_g_1))/cos(pitch_g_1))*Mpu6050_Data.Gyro_Y + (cos(roll_g_1)*sin(pitch_g_1))/cos(pitch_g_1)*Mpu6050_Data.Gyro_Z)*dt;//
	...
	//计步
	if((pitch_g < -10 ) && state==0)state = 1;
	else if(pitch_g > -10 && pitch_g < 10 && state==1)state = 2;
	else if(( pitch_g > 10) && state==2)state = 3;
	
	if(state == 3)
	{
		step_num++;
		update_step_count_label(step_num);
		state = 0;
	}
...
//	update_chart_value_2(pitch_g+180);
